...
Attacklab.showdown.converter=function(){var i,j,n,o=0;this.makeHtml=function(a){i=[];j=[];n=[];a=a.replace(/~/g,"~T");a=a.replace(/\$/g,"~D");a=a.replace(/\r\n/g,"\n");a=a.replace(/\r/g,"\n");a=u("\n\n"+a+"\n\n");a=a.replace(/^[ \t]+$/mg,"");a=v(a);a=B(a);a=p(a);a=w(a);a=a.replace(/~D/g,"$$");return a=a.replace(/~T/g,"~")};var B=function(a){return a=a.replace(/^[ ]{0,3}\[(.+)\]:[ \t]*\n?[ \t]*<?(\S+?)>?[ \t]*\n?[ \t]*(?:(\n*)["(](.+?)[")][ \t]*)?(?:\n+)/gm,function(a,c,b,e,g){c=c.toLowerCase();i[c]=
...
e;g++)b+=" ";return b});a=a.replace(/~A/g,"    ");return a=a.replace(/~B/g,"")},k=function(a,d,c){d="(["+d.replace(/([\[\]\\])/g,"\\$1")+"])";c&&(d="\\\\"+d);return a=a.replace(RegExp(d,"g"),q)},q=function(a,d){return"~E"+d.charCodeAt(0)+"E"}};var Showdown=Attacklab.showdown;Attacklab.fileLoaded&&Attacklab.fileLoaded("showdown.js");